      ...
      <div class="position-relative">
        ...
        <div class="form-group d-sm-flex mt-2r row">
          <div class="col-sm-6">
            ...
            >
              <option v-for="(val, i) in physicaleducation" :key="i">
                {{ val.subject_name }}
              </option>
            </select>
...
export default {
  name: "EditPhysicalEducationCourse",
  components: {
    "v-select": vSelect,
  },
  data() {
  ...
  props: ["physicaleducation", "transcripts", "student_id","courses_id","transcript_id"],
  methods: {
    addCourses() {
       axios.post(route("editPhysicaEducationCourse.store"), this.form)
     .then((response) => {
        window.location =
          "/edit-health/" + this.student_id + "/" + this.transcript_id;
      }).catch(error => {
