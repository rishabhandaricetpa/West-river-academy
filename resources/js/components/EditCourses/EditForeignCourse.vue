      ...
      <div class="position-relative">
        ...
        <div class="form-group d-sm-flex mt-2r row">
          <div class="col-sm-6">
            ...
            >
              <option v-for="(val, i) in foreigncourse" :key="i">
                {{ val.subject_name }}
              </option>
            </select>
...
export default {
  ...
  props: ["foreigncourse", "transcripts", "student_id","courses_id","transcript_id"],
  methods: {
    addCourses() {
       axios.post(route("editForeign.store"), this.form)
     .then((response) => {
        window.location =
