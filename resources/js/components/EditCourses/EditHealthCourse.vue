     ...
     <span
        ...
        <div class="form-group d-sm-flex mt-2r row">
          <div class="col-sm-6">
            ...
            >
              <option v-for="(val, i) in healthcourse" :key="i">
                {{ val.subject_name }}
              </option>
            </select>
...
export default {
  ...
  props: ["healthcourse", "transcripts", "student_id","courses_id","transcript_id"],
  methods: {
    addCourses() {
       axios.post(route("editHealthCourse.store"), this.form)
     .then((response) => {
        window.location =
          "/edit-foreign/" + this.student_id + "/" + this.transcript_id;
      }).catch(error => {
