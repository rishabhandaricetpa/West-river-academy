...
@section('content')
    ...
    <main class="position-relative container form-content mt-4">
        ...
        <div class="form-wrap border bg-light py-5 px-25">
            <h2>Edit Student</h2>
            <div id="app">
                <edit-enroll :semesters='@json($countryData)' :date_of_birth='@json($birth)' :students='@json($studentData)'
                    :periods='@json($enrollPeriods)' :semmonth='@json($semestermonth)'></edit-enroll>
            </div>
    </main>
    <!-- Choose Dates -->
            ...
            <div class="modal-content">
                <div class="modal-body">
                    <p>Annual enrollment covers the 12 months from
                        {{ Carbon\Carbon::parse($countryData->start_date)->format('F d') }} -
                        {{ Carbon\Carbon::parse($countryData->end_date)->format('F d') }}.
                        Second Semester Only covers {{ Carbon\Carbon::parse($semestermonth)->format('F d') }} -
                        {{ Carbon\Carbon::parse($countryData->end_date)->format('F d') }}.
