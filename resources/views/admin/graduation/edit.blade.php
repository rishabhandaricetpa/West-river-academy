    ...
    <form action="{{ route('admin.update.graduation',$graduation->id) }}" class="row" method="post"
      ...
      <div class="form-group col-sm-6">
        ...
        <input type="datepicker" class=" datepicker form-control" id="grad_date"
          value="{{ Carbon\Carbon::parse($graduation->details->grad_date)->format('M j, Y') }}" name="grad_date">
      </div>
      ...
      <div class="form-group col-sm-6">
        ...
        <textarea name="situation" class="w-100" id="situation" cols="80"
          rows="4" maxlength="2000" onKeyPress="if(this.value.length==2000) return false;">{{ $graduation->details->situation }}</textarea>
      </div>
      <div class=" form-group col-sm-6">
