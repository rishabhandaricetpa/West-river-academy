                    ...
                    <div class="form-group d-sm-flex mb-2">
                        ...
                        <div>
                            <input type="text" name="graduation_date"
                                value="{{ Carbon\Carbon::parse($dateofGraduation->date_of_graduation)->format('M d Y') }}"
            ...
            @foreach ($transcriptData as $school)
                <div class="form-wrap border bg-light py-5 px-25 mb-4">
                    ...
                    <table id="addressData" class="table table-bordered table-striped data-table mt-3">
                        <a type=" button" href="{{ route('admin.deleteSchool9_12', $school->id) }}"
                            class="btn btn-primary transform-none">Delete this Year from Transcript</a>
                            ...
                            @foreach ($school->TranscriptCourse9_12 as $course)
                                @foreach ($course->subjects as $subject)
                                    <tr>
                                        ...
                                        <td>{{ $course->score }}</td>
                                        <td><a type=" button"
                                                href="{{ route('admin.edit.subGrades9_12', [$subject->id, $school->transcript_id, $school->grade]) }}"
