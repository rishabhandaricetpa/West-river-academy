          ...
          <!-- /.card-header -->
            <table id="example1" class="table table-bordered table-striped data-table">
              ...
              <tbody>
                ...
                <tr>
                  @if ($isAdmin)
                  <td> <input type="checkbox" value="{{ $data->id }}" name="is_archived" onclick="archieve(this.value)">
                  </td>
                  @endif
                  ...
                  @if ($data->created_at->diffInDays($date) > 6 && $data['recordTransfer']['request_status'] !== 'Record
                  Received')
                  <td class="btn btn-primary"><a
                      href="{{ route('admin.student.schoolRecord', [$data->student_profile_id, $data['recordTransfer']['id']]) }}">Alert:Resend
                      Request For Record Transfer</a></td>
                  @else
                  <td><a
                  ...
                  @if ($isAdmin)
                  <td><a href="javascript:void(0)" data-id="{{ $data->id }}" onclick="editDashboard(event.target)"
                      data-toggle="modal" data-target="#assignRecord" class="btn btn-primary">Assign</a></td>
                  @endif
                  ...
                  <td><a href="javascript:void(0)" data-id="{{ $data->id }}"
                      ...
                      data-target="#assignStatusToRecord" class="btn btn-primary">Status</a></td>
                  @endif
